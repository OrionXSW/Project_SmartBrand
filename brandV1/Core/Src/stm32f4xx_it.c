...
void USART1_IRQHandler(void)
{
  /* USER CODE BEGIN USART1_IRQn 0 */
    uint8_t temp;
    if(__HAL_UART_GET_FLAG(&huart1,UART_FLAG_RXNE))     // 接收中断
    {
        temp = huart1.Instance->DR;
        uart1.databuff[uart1.length++] = temp;
    }
    
    if(__HAL_UART_GET_FLAG(&huart1,UART_FLAG_IDLE))      // 空闲中断
    {
        huart1.Instance->SR;
        huart1.Instance->DR;    // 
        ...
        {
            uart1.flag_over = 1;
            uart1.databuff[uart1.length] = '\0';
            uart1.length = 0;
            UART_SendString(uart1.databuff);
